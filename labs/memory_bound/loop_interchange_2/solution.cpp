...
static void filterVertically(uint8_t *output, const uint8_t *input,
  ...
  int constexpr c_step = 4;

  const int rounding = 1 << (shift - 1);

  for (int c = 0; c < width; c+=c_step) {
    ...
    for (int r = 0; r < std::min(radius, height); r++) {
      ...
      for (int y = 0; y <= std::min(r + radius, height - 1); y++) {
        ...
        for (int c_i = 0; c_i < c_step; c_i++) {
          dot[c_i] += input[y * width + (c + c_i)] * weight;
          sum[c_i] += weight;
        }
      }
      ...
      int value[c_step] = {};
      for (int c_i = 0; c_i < c_step; c_i++) {
        value[c_i] = static_cast<int>(dot[c_i] / static_cast<float>(sum[c_i]) + 0.5f);;
        output[r * width + c + c_i] = static_cast<uint8_t>(value[c_i]);
      }
    }
    ...
    for (int r = radius; r < height - radius; r++) {
      ...
      for (int i = 0; i < radius + 1 + radius; i++) {
        ...
        for (int c_i = 0; c_i < c_step; c_i++) {
          dot[c_i] += input[(r - radius + i) * width + c + c_i] * kernel[i];
        }
      }

      int value[c_step] = {};
      for (int c_i = 0; c_i < c_step; c_i++) {
        value[c_i] = (dot[c_i] + rounding) >> shift;
        output[r * width + c + c_i] = static_cast<uint8_t>(value[c_i]);
      }
