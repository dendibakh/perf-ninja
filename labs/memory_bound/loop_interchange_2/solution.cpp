...
static void filterVertically(uint8_t *output, const uint8_t *input,
  ...
  for (int r = 0; r < std::min(radius, height); r++) {
    ...
    for (int y = 0; y <= std::min(r + radius, height - 1); y++) {
      int weight = *p++;
      for (int c = 0; c < width; c++) {
        temp_dot[c] += input[y * width + c] * weight;
      }
      sum += weight;
    }
    // Normalize accumulators for columns and write result
    for (int c = 0; c < width; c++) {
      int value = static_cast<int>(temp_dot[c] / static_cast<float>(sum) + 0.5f);
  ...
  for (int r = radius; r < height - radius; r++) {
    ...
    for (int i = 0; i < radius + 1 + radius; i++) {
      for (int c = 0; c < width; c++) {
        temp_dot[c] += input[(r - radius + i) * width + c] * kernel[i];
      }
    }

    for (int c = 0; c < width; c++) {
      // Fast shift instead of division
      int value = (temp_dot[c] + rounding) >> shift;
      output[r * width + c] = static_cast<uint8_t>(value);
    }
  }
  ...
  for (int r = std::max(radius, height - radius); r < height; r++) {
    ...
    // Normalize column accumulators and write result
    for (int c = 0; c < width; c++) {
      int value = static_cast<int>(temp_dot[c] / static_cast<float>(sum) + 0.5f);
      output[r * width + c] = static_cast<uint8_t>(value);
    }
