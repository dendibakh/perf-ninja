...
static void filterVertically(uint8_t *output, const uint8_t *input,
    ...
    constexpr int MAX_WIDTH =
        32 * 1024;  // Honestly a bit unclear what max dim is
    ...
    for (int r = 0; r < std::min(radius, height); r++) {
        ...
        std::memset(accumulator, 0, sizeof(int) * width);
        ...
        for (int c = 0; c < width; c++) {
            int value = static_cast<int>(
                accumulator[c] / static_cast<float>(sum) + 0.5f);
    ...
    for (int r = radius; r < height - radius; r++) {
        ...
        for (int i = 0; i < radius + 1 + radius; i++) {
            for (int c = 0; c < width; c++) {
                accumulator[c] +=
                    input[(r - radius + i) * width + c] * kernel[i];
            }
        }
