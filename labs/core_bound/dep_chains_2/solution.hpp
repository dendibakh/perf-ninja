...
void randomParticleMotion(std::vector<Particle> &particles, uint32_t seed) {
  ...
  for (int i = 0; i < STEPS; i++) {
    for (int j = 0; j < N; j++) {
      degree_angles[j] = rng.gen();
    }
    int t = 0;
    for (; t + kBatch - 1 < particles.size(); t += kBatch) {
      __m256i angle = _mm256_loadu_si256((__m256i*)(degree_angles + t));
      angle = _mm256_mul_ps(angle, degree_to_radian_bc);
      ...
      batch_sine = _mm256_mul_ps(batch_sine, batch_v);
      batch_cosine = _mm256_mul_ps(batch_cosine, batch_v);

      batch_x = _mm256_add_ps(batch_x, batch_cosine);
      batch_y = _mm256_add_ps(batch_y, batch_sine);
