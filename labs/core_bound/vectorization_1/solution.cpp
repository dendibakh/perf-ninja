...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  simd_sequence_t simd1 = transpose(sequences1);
  simd_sequence_t simd2 = transpose(sequences2);
  ...
  /*
    ...
    */
  for (unsigned col = 1; col <= simd2.size(); ++col) {
    ...
    for (unsigned row = 1; row <= simd1.size(); ++row) {
      // Compute next score from diagonal direction with match/mismatch.
      score_t best_cell_score;
      for (size_t k = 0; k < sequence_count_v; ++k)
        best_cell_score[k] =
          last_diagonal_score[k] +
          (simd1[row - 1][k] == simd2[col - 1][k] ? match[k] : mismatch[k]);
