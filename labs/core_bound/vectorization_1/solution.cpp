...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  for (size_t sequence_idx = 0; sequence_idx < sequences1.size();
    ...
    for (unsigned col = 1; col <= sequence2.size(); ++col) {
      column_t best_cell_column = score_column;
      best_cell_column[0] = horizontal_gap_column[0];
      ...
      for (unsigned row = 1; row <= sequence1.size(); ++row) {
        best_cell_column[row] = std::max(best_cell_column[row], last_vertical_gap);
        last_vertical_gap = std::max(last_vertical_gap + gap_extension, best_cell_column[row] + gap_open);
      }
      score_column = best_cell_column;
        // Compute the next values for vertical and horizontal gap.
      for (unsigned row = 1; row <= sequence1.size(); ++row) {
        horizontal_gap_column[row] =
            std::max(horizontal_gap_column[row] + gap_extension, score_column[row] + gap_open);
      }
    }
