...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  for (size_t sequence_idx = 0; sequence_idx < sequences1.size();
    ...
    column_t horizontal_gap_column{gap_open};
    column_t horizontal_gap_row{gap_open};
    column_t vertical_gaps{};
    ...
    column_t previous_row_horizontal_gap {horizontal_gap_column[0]};
    column_t previous_row_vertial_gap {static_cast<score_t>(horizontal_gap_column[0] + gap_open)};
    column_t previous_score_row {horizontal_gap_column[0]}; // initial values are is gap_open

    #pragma clang loop vectorize(enable)
    for (size_t i = 1; i < score_column.size(); ++i) 
    {
      previous_score_row[i] = score_column[0] + ((i-1) * gap_extension);
      previous_row_vertial_gap[i] = horizontal_gap_column[0] + gap_open + ((i-1) * gap_extension);
      previous_row_horizontal_gap[i] = horizontal_gap_column[0] + (i * gap_extension);
    }
    /*
    ...
    */
    ...
    for (unsigned col = 1; col <= sequence2.size(); ++col) {
      ...
      score_t last_vertical_gap = horizontal_gap_column[0] + gap_open;
      horizontal_gap_column[0] += gap_extension; // so every new COLUMN increments with gap_extension

      // So rather than checking the previous scores with the last_vertical_gap and horizontal_gap_column for its row
      // it should compare to the previous row, indexing into the column
