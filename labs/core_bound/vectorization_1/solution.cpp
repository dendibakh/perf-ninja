...
static simd_sequence_t transpose(std::vector<sequence_t> const &sequences) {
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  const simd_sequence_t trSeq1 = transpose(sequences1);
  const simd_sequence_t trSeq2 = transpose(sequences2);
