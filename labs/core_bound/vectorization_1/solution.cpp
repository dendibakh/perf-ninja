...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  for (size_t sequence_idx = 0; sequence_idx < sequences1.size();
    ...
    for (unsigned col = 1; col <= sequence2.size(); ++col) {
      ...
      for (unsigned row = 1; row <= sequence1.size(); ++row) {
        score_column[row] = std::max(last_vertical_gap, score_column[row]);
        last_vertical_gap = std::max((score_t)(last_vertical_gap + gap_extension), (score_t)(score_column[row] + gap_open));
      }
      for (unsigned row = 1; row <= sequence1.size(); ++row) {
        horizontal_gap_column[row] += gap_extension;
        horizontal_gap_column[row] =
            std::max(horizontal_gap_column[row], (score_t)(score_column[row] + gap_open));
      }
    }
