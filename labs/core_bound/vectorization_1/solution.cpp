...
using sequence_row = std::array<int16_t, sequence_count_v>;
using sequence_table = std::array<sequence_row, sequence_size_v>;

sequence_table transpose(const std::vector<sequence_t> &sequences) {
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  const auto trSeq1 = transpose(sequences1);
  const auto trSeq2 = transpose(sequences2);

  using score_t = sequence_row;
