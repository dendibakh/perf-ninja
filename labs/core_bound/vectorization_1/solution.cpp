                           ...
                           std::vector<sequence_t> const &sequences2)
{
   ...
   */
  for (unsigned col = 1; col <= sequences2_transposed.size(); ++col)
  {
    ...
    for (unsigned row = 1; row <= sequences1_transposed.size(); ++row)
    {
      // Compute next score from diagonal direction with match/mismatch.
      score_t best_cell_score = last_diagonal_score;
      for (size_t k = 0; k < sequence_count_v; ++k)
      {
        best_cell_score[k] += (sequences1_transposed[row - 1][k] == sequences2_transposed[col - 1][k] ? match[k] : mismatch[k]);
      }
