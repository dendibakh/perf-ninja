...
simd_sequence_t transpose(std::vector<sequence_t> const &sequences) {
  ...
  for (size_t sequence_idx = 0; sequence_idx < sequences.size();
       ++sequence_idx) {
    for (size_t element_idx = 0; element_idx < sequences[sequence_idx].size();
         ++element_idx) {
      simd_sequence[element_idx][sequence_idx] = sequences[sequence_idx][element_idx];
    }
  }
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  /*
    ...
    */
  for (unsigned col = 1; col <= trSeq1.size(); ++col) {
    ...
    for (unsigned row = 1; row <= trSeq2.size(); ++row) {
      // Compute next score from diagonal direction with match/mismatch.
      score_t best_cell_score = last_diagonal_score;
      for (size_t k = 0; k < sequence_count_v; ++k){
        best_cell_score[k] += (trSeq1[col - 1][k] == trSeq2[row - 1][k] ? match[k] : mismatch[k]);
      }
