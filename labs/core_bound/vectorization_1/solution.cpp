...
// Transpose a collection of sequences into a sequence of simd vectors.
simd_sequence_t transpose(std::vector<sequence_t> const &sequences){
  assert(sequences.size() == sequence_count_v &&
         "The number of sequences must match the sequence count.");
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
   ...
   */
  score_t gap_open{};
  gap_open.fill(-11); // Initialize all elements to -11
  score_t gap_extension{};
  gap_extension.fill(-1); // Initialize all elements to -1
  score_t match{};
  match.fill(6); // Initialize all elements to 6
  score_t mismatch{};
  mismatch.fill(-4); // Initialize all elements to -4
