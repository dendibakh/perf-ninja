  ...
  for (size_t sequence_idx = 0; sequence_idx < sequences1.size();
    ...
    for (unsigned col = 1; col <= sequence2.size(); ++col) {
      ...
      score_t last_diagonal_score =
          col == 1 ? 0 : gap_open + (col - 2) * gap_extension;
      score_t last_vertical_gap = (col - 1) * gap_extension + 2 * gap_open;

      constexpr unsigned unroll = 4;
      for (unsigned orow = 1; orow <= sequence1.size(); orow += unroll) {
        score_t tmp[unroll];
        for (unsigned row = orow;
             row <= std::min(orow + unroll - 1,
                             static_cast<unsigned>(sequence1.size()));
             row++) {
          auto i = row - orow;
        ...
        for (unsigned row = orow;
             ...
             row++) {
          auto i = row - orow;
          score_t best_cell_score = tmp[i];
          // Determine best score from diagonal, vertical, or horizontal
          // direction.
          best_cell_score = std::max(best_cell_score, last_vertical_gap);
          best_cell_score =
              std::max(best_cell_score, horizontal_gap_column[row]);
