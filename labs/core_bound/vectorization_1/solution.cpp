...
simd_sequence_t transpose(std::vector<sequence_t> const &sequences){
  ...
  // NOTE: it is also possible to transpose a matrix in place, this is not it.
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  //      ++sequence_idx) {
  using score_t = simd_score_t; // int16_t;
  ...
  */
  ...
  for (size_t i = 1; i < score_column.size(); ++i) {
    for (size_t k = 0; k < sequence_count_v; ++k) {  // vectorization
